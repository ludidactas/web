...
import { getIdSalaDeProfe } from '@/wss/salas/db'
...
export default async function SalasPage() {
  ...
  const idSalaExistente = session?.user?.email ? await getIdSalaDeProfe(session.user.email) : null

  return (
    <ConexionProfeProvider auth={{ token }} autoConectar={!!idSalaExistente}>
      <Toaster />
