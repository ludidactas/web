...
function DisplayEncuesta({ encuesta }: { encuesta: Encuesta }) {
  ...
  return (
    <div className={`p-2 mx-4  mb-2 rounded-3xl`}>
      ...
      <div
        className="flex flex-col sm:flex-row md:gap-4 bg-[#00B0D2]/10 text-[#00B0D2] border-4 border-[#00B0D2]/30 rounded-full p-2 md:px-4 justify-between sm:items-center cursor-pointer hover:bg-[#00B0D2]/20 transition-colors"
      ...
      >
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Icono de despliegue */}
          {isExpanded ? (
            <Iconito className="h-10 w-10" icon={"tabler:circle-chevron-up"} />

          ) : (
            <Iconito className="h-10 w-10" icon={"tabler:circle-chevron-down"} />
          )}
        </div>
