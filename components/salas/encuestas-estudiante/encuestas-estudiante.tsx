...
function DisplayEncuesta({ encuesta }: { encuesta: EncuestaHidratada }) {
  ...
  return (
    <div className={`flex flex-col w-full md:w-2/3 gap-4 md:p-8 border-indigo-500/70 border-2 p-4 md:border-4 rounded-xl ${yaVotado? 'border-slate-300':''}`}>
      {/* Titulo y opciones */}
      <div className="flex gap-4 md:gap-6 items-start justify-between rounded-xl">
        <div
          className={`flex items-center text-indigo-500  gap-2 md:gap-4 ${yaVotado ? 'grayscale' : ''}`}
        >
