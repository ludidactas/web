...
interface LdFixtures {
  ...
  setupSala: (profe: LoginUser) => Promise<{ sala: Page; estudiante: (estudiante: LoginEstudiante) => Promise<Page> }>
}
...
export async function armarSala(browser: Browser, profe: LoginUser) {
  ...
  const estudiante = async ({ nombre, dni }: LoginEstudiante) => {
    ...
    await alumnoPage.getByPlaceholder('Ingresá tu DNI').fill(dni)
...
export const test = base.extend<LdFixtures>({
  ...
  setupSala: async ({ browser }, use) => {
    // eslint-disable-next-line react-hooks/rules-of-hooks
    await use(async (profe: LoginUser) => {
      return await armarSala(browser, profe)
    })
  },
})
